...
export async function sendNodeMessage(userPhoneNumber, business_phone_number_id) {
    ...
    // Handle delay FIRST before duplicate check (delayed messages are legitimate)
    let delay;
    if (flowVersion === 2) {
        ...
        if (delay !== undefined && delay > 0) {
            ...
            // Save session before timeout to persist the cleared delay
    ...
    // Prevent race conditions from causing same node to be processed multiple times
    // This is critical for peak demand to avoid sending duplicate messages to users
    const now = Date.now();
    const lastProcessed = userSession.lastProcessedNode;
    const lastProcessedTime = userSession.lastProcessedTime || 0;

    // If same node was processed within last 3 seconds, skip to prevent duplicates
    if (lastProcessed === currNode && (now - lastProcessedTime) < 3000) {
        console.log(`⚠️ [DUPLICATE PREVENTION] Skipping node ${currNode} - was processed ${now - lastProcessedTime}ms ago`);
        return;
    }

    // Mark this node as being processed NOW (before any message sends)
    // Save immediately to prevent concurrent calls from bypassing this check
    userSession.lastProcessedNode = currNode;
    userSession.lastProcessedTime = now;
    await userSessions.set(key, userSession);
    console.log(`✅ [DUPLICATE PREVENTION] Marked node ${currNode} as processing at ${now}`)
    ...
    // NOTE: Session saves are handled by processNodeLegacy/processNodeV2 themselves.
    // Do NOT save here - for recursive calls (auto-advancing nodes like string, image, etc.),
    // this would overwrite the correctly saved session (with inputVariable set) from the
    // inner recursive call with the outer call's stale session (inputVariable = null).
    // This was causing the bug where 'name' and 'calibration' variables weren't being saved.
}
...
async function processNodeV2(userSession, userPhoneNumber, business_phone_number_id) {
    ...
    switch (nodeType) {
        ...
        case "sendMessage":
            ...
            if (messageType === "text") {
                ...
                // Auto-advance - save session BEFORE recursive call to prevent duplicates
                if (nextNode.length > 0) {
                    userSession.currNode = nextNode[0];
                    await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
                    await sendNodeMessage(userPhoneNumber, business_phone_number_id);
                    return; // Exit to prevent final save from overwriting
                }
            }
    ...
    // Final save for non-recursive cases (like askQuestion)
    await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
}
...
async function processNodeLegacy(userSession, userPhoneNumber, business_phone_number_id) {
    ...
    if (currNode !== undefined && currNode !== null) {
        ...
        switch (flow[currNode]?.type) {
            case "Button":
                ...
                if (variable) {
                    ...
                    userSession.inputVariableType = flow[currNode]?.dataType || 'text';
                    console.log(`[snm] Button node set inputVariable="${variable}" for node ${currNode}`)
                    await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
                } else {
                    console.log(`⚠️ [snm] Button node ${currNode} has NO variable property configured`);
                }
            ...
            case "List":
                ...
                if (variable) {
                    userSession.inputVariable = variable
                    userSession.inputVariableType = flow[currNode]?.dataType || 'text';
                    console.log(`[snm] List node set inputVariable="${variable}" for node ${currNode}`)
                    await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
                } else {
                    console.log(`⚠️ [snm] List node ${currNode} has NO variable property configured`);
                }
            ...
            case "Text":
                ...
                if (variable) {
                    userSession.inputVariable = variable
                    userSession.inputVariableType = flow[currNode]?.dataType || 'text';
                    console.log(`[snm] Text node set inputVariable="${variable}" for node ${currNode}`)
                    // Save immediately to ensure inputVariable is persisted
                    await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);

                    // Verify the save worked
                    const verifySession = await userSessions.get(userPhoneNumber + business_phone_number_id);
                    if (verifySession?.inputVariable !== variable) {
                        console.error(`❌ [snm] inputVariable NOT persisted! Expected "${variable}", got "${verifySession?.inputVariable}"`);
                    } else {
                        console.log(`✅ [snm] inputVariable="${variable}" verified in Redis`);
                    }
                } else {
                    console.log(`⚠️ [snm] Text node ${currNode} has NO variable property configured`);
                }
            ...
            case "string":
                ...
                if (!nextNode || nextNode.length === 0) {
                    console.log(`🏁 [snm] string node reached terminal - flow complete`);
                    userSession.flowCompleted = true;
                    userSession.currNode = currNode; // Stay on last node
                    await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
                    // Don't send completion message here - just end cleanly
                    // User's next message will trigger the restart option
                    break;
                }

                userSession.currNode = nextNode[0];
                // CRITICAL FIX: Update nextNode before save so recursive call has correct value
                userSession.nextNode = adjListParsed[userSession.currNode] || [];
                console.log(`[snm] string auto-advance to node ${userSession.currNode}`)

                await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
                await sendNodeMessage(userPhoneNumber, business_phone_number_id);
                return; // Return to prevent final save from overwriting nextNode
            ...
            case "image":
                ...
                // Check if this is a terminal node
                if (!nextNode || nextNode.length === 0) {
                    console.log(`🏁 [snm] image node reached terminal - flow complete`);
                    userSession.flowCompleted = true;
                ...
                userSession.currNode = nextNode[0];
                // CRITICAL FIX: Update nextNode before save
                userSession.nextNode = adjListParsed[userSession.currNode] || [];
            ...
            case "audio":
                ...
                // Check if this is a terminal node
                if (!nextNode || nextNode.length === 0) {
                    console.log(`🏁 [snm] audio node reached terminal - flow complete`);
                    userSession.flowCompleted = true;
                    userSession.currNode = currNode;
                    await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
                    break;
            ...
            case "video":
                ...
                // Check if this is a terminal node
                if (!nextNode || nextNode.length === 0) {
                    console.log(`🏁 [snm] video node reached terminal - flow complete`);
                    userSession.flowCompleted = true;
                    userSession.currNode = currNode;
                    await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
                    break;
            ...
            case "location":
                ...
                // Check if this is a terminal node
                if (!nextNode || nextNode.length === 0) {
                    console.log(`🏁 [snm] location node reached terminal - flow complete`);
                    userSession.flowCompleted = true;
                    userSession.currNode = currNode;
                    await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
                    break;
                ...
                // CRITICAL FIX: Update nextNode before save
                userSession.nextNode = adjListParsed[userSession.currNode] || [];
                console.log("location currNode: ", userSession.currNode)
                await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
                await sendNodeMessage(userPhoneNumber, business_phone_number_id);
                return; // Return to prevent final save from overwriting nextNode
            ...
            case "api":
                ...
                // Check if this is a terminal node
                if (!nextNode || nextNode.length === 0) {
                    console.log(`🏁 [snm] api node reached terminal - flow complete`);
                    userSession.flowCompleted = true;
                    userSession.currNode = currNode;
                    await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
                    break;
                ...
                // CRITICAL FIX: Update nextNode before save
                userSession.nextNode = adjListParsed[userSession.currNode] || [];
                console.log("api currNode: ", userSession.currNode)
                await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
                await sendNodeMessage(userPhoneNumber, business_phone_number_id);
                return; // Return to prevent final save from overwriting nextNode
            ...
            case "template":
                ...
                // Check if this is a terminal node
                if (!nextNode || nextNode.length === 0) {
                    console.log(`🏁 [snm] template node reached terminal - flow complete`);
                    userSession.flowCompleted = true;
                    userSession.currNode = currNode;
                    await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
                    break;
            ...
            case "custom":
                ...
                // Check if this is a terminal node
                if (!nextNode || nextNode.length === 0) {
                    console.log(`🏁 [snm] custom node reached terminal - flow complete`);
                    userSession.flowCompleted = true;
                    userSession.currNode = currNode;
                    await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
                    break;
            ...
            case "customint":
                ...
                try {
                    ...
                    // Check if this is a terminal node
                    if (!nextNode || nextNode.length === 0) {
                        console.log(`🏁 [snm] customint node reached terminal - flow complete`);
                        userSession.flowCompleted = true;
                        userSession.currNode = currNode;
                        await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
                        return; // Return early for terminal node
                    }
                    ...
                    // Check if this is a terminal node
                    if (!nextNode || nextNode.length === 0) {
                        console.log(`🏁 [snm] customint node (on error) reached terminal - flow complete`);
                        userSession.flowCompleted = true;
                        userSession.currNode = currNode;
                        await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
                        return; // Return early for terminal node
            ...
            case "flowjson":
                ...
                // Check if this is a terminal node
                if (!nextNode || nextNode.length === 0) {
                    console.log(`🏁 [snm] flowjson node is terminal - flow complete after user response`);
                    userSession.flowCompleted = true;
                    userSession.currNode = currNode;
                    userSession.nextNode = [];
                } else {
                    userSession.currNode = nextNode[0];
                    // CRITICAL FIX: Set nextNode correctly based on new currNode
                    userSession.nextNode = adjListParsed[userSession.currNode] || [];
                }
                await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
                console.log(`✅ FlowJSON advanced to next node: ${userSession.currNode} (will process after nfm_reply)`);
                // Return early to prevent final save from overwriting nextNode
                return;
        ...
        userSession.nextNode = nextNode;
        // Log inputVariable state before final save
        console.log(`[snm] Final save: currNode=${userSession.currNode}, inputVariable="${userSession.inputVariable || 'null'}"`);
        await userSessions.set(userPhoneNumber + business_phone_number_id, userSession);
        console.log("Updated Current Node: ", userSession.currNode);
        // await Promise.all([sendMessagePromise, sendDynamicPromise])
    } else {
        // Only reset to start if flow is NOT completed
        if (!userSession.flowCompleted) {
            userSession.currNode = userSession.startNode;
            userSession.nextNode = adjListParsed[userSession.currNode] || [];
            console.log("Resetting to start node: ", userSession.currNode);
        } else {
            console.log("Flow is completed, not resetting to start");
        }
    }
}
