...
class SessionManager {
  constructor() {
    ...
    this.SESSION_TTL = 86400; // 24 hours in seconds (default)

    // Tenant-specific TTL overrides (in seconds)
    this.TENANT_TTL_OVERRIDES = {
      'ehgymjv': 86400  // 24 hours for ehgymjv (interview flow)
    };
  }
  ...
  async set(key, value) {
    try {
      ...
      // Determine TTL based on tenant
      const tenant = value?.tenant;
      const ttl = tenant && this.TENANT_TTL_OVERRIDES[tenant]
        ? this.TENANT_TTL_OVERRIDES[tenant]
        : this.SESSION_TTL;

      if (tenant && this.TENANT_TTL_OVERRIDES[tenant]) {
        console.log(`⏰ Using extended TTL for tenant "${tenant}": ${ttl} seconds (${ttl/3600} hours)`);
      }
  ...
  async cleanupInactiveSessions(inactivityThreshold = 30 * 60 * 1000) {
    try {
      ...
      // Tenant-specific inactivity thresholds (in milliseconds)
      const TENANT_INACTIVITY_OVERRIDES = {
        'ehgymjv': 24 * 60 * 60 * 1000  // 24 hours for ehgymjv (interview flow)
      };

      for (const [key, session] of entries) {
        if (!session.lastActivityTime) continue;

        // Determine threshold based on tenant
        const tenant = session.tenant;
        const threshold = tenant && TENANT_INACTIVITY_OVERRIDES[tenant]
          ? TENANT_INACTIVITY_OVERRIDES[tenant]
          : inactivityThreshold;
        ...
        if (inactiveTime > threshold) {
          ...
          cleanedCount++;
        } else if (tenant && TENANT_INACTIVITY_OVERRIDES[tenant]) {
          // Session is protected by tenant-specific threshold
          protectedCount++;
        }
      }
      ...
      if (protectedCount > 0) {
        console.log(`🛡️  Protected ${protectedCount} session(s) with extended inactivity threshold`);
      }
