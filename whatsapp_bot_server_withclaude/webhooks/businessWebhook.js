...
import { setAgentMapping, getAgentMapping, isAgent, isCustomerMapped } from "../helpers/agentMapping.js";
...
export async function businessWebhook(req, res) {
    ...
    if(messageType == "interactive"){
      ...
      const isAgentAssigned = await isCustomerMapped(recipient);
      ...
      if(selectionId.startsWith('chatwith')){
        if(!isAgentAssigned){
          ...
          await setAgentMapping(userPhoneNumber, recipient)
      ...
      else if(await isAgent(userPhoneNumber)) {
...
async function handleCommands(message_text, userSession){
    ...
    switch(command){
      case "button":
        ...
        recipient = await getAgentMapping(userSession.userPhoneNumber)
        break
      case "template":
