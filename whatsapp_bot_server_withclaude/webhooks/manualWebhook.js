...
import { getAgentMapping, deleteAgentMapping, isAgent } from "../helpers/agentMapping.js";
...
export async function manualWebhook(req, userSession){
  ...
  if (await isAgent(userPhoneNumber)){
    recipient = await getAgentMapping(userPhoneNumber)
    if (message_text == "close"){
      ...
      await deleteAgentMapping(userPhoneNumber)
      delete customer_userSession?.talking_to
      console.log("Agent mapping deleted for: ", userPhoneNumber)
