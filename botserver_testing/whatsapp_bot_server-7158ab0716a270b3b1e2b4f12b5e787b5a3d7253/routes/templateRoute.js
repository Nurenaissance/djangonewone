  ...
  try {
    ...
    setImmediate(async () => {
      try {
        ...
        if (type === 'campaign') {
          await sendCampaign(validation.data, access_token, tenant_id, account_id, bpid);
        } else if (type === 'template') {
          await sendTemplate(validation.data, access_token, tenant_id, account_id, bpid);
        } else if (type === 'group') {
          await sendTemplateToGroup(validation.data, access_token, tenant_id, account_id, bpid);
        }
